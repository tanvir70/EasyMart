...
public class JDBCTemplate {
    ...
    public <E> List<E> queryForObject(
            String query, ObjectRowMapper<E> objectRowMapper) {
        try (var connection = dataSource.getConnection();
             var statement = connection.prepareStatement(query)) {
        ...
        for (Object parameter : parameters) {
            if (parameter instanceof String) {
                ...
                preparedStatement.setDouble(
                        idx, (Double) parameter);
            } else if (parameter instanceof LocalDateTime) {
                preparedStatement.setTimestamp(idx,
                        Timestamp.valueOf(
                ...
                preparedStatement.setBigDecimal(
                        idx, (BigDecimal) parameter);
            } else if (parameter instanceof Boolean) {
                preparedStatement.setBoolean(
                        idx, (Boolean) parameter);
            }
            idx++;
        }
    }
    ...
    public void deleteById(String query, Long id) {
        try (var connection = dataSource.getConnection();
            ...
            LOGGER.error(
                    "Unable to execute delete by id: {}", id, e);
            throw new RuntimeException(
                    "Unable to execute delete", e);
        }
    }
